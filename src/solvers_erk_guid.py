#----------------------------------------------------------------------------
# Modified diff-sampler/diff-solvers-main/solvers.py for ERK-Guid (ICLR 2026)
# Paper: "Error as Signal: Stiffness-Aware Diffusion Sampling via Embedded Runge-Kutta Guidance"
# 
# [!] NOTE: The ERK-Guid mechanism (via `w_stiff` and `w_con`) is currently 
# implemented ONLY for the following solvers in this script:
#   1. Heun's 2nd order (heun_sampler)
#   2. DPM-Solver-2 (dpm_2_sampler)
#   3. DEIS (deis_sampler)
#----------------------------------------------------------------------------
    ...
    **kwargs
    ...
    for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):                # 0, ..., N-1
        ...
        if use_afs:
            ...
            denoised = get_denoised(net, x_cur, t_cur, class_labels=class_labels, condition=condition, unconditional_condition=unconditional_condition)
            d_cur = (x_cur - denoised) / t_cur
        x_next_Euler = x_cur + (t_next - t_cur) * d_cur

        # Apply 2nd order correction.
        denoised = get_denoised(net, x_next_Euler, t_next, class_labels=class_labels, condition=condition, unconditional_condition=unconditional_condition)
        d_prime = (x_next_Euler - denoised) / t_next
        x_next = x_cur + (t_next - t_cur) * (0.5 * d_cur + 0.5 * d_prime)

        # Apply ERK-Guid
        if w_stiff > 0.0:
            if x_cur_embedded is not None:
                stiffness = ((d_cur - d_cur_embedded)*(d_cur - d_cur_embedded)).sum(dim=[1,2,3],keepdim=True).sqrt() / (((x_cur - x_cur_embedded)*(x_cur - x_cur_embedded)).sum(dim=[1,2,3],keepdim=True).sqrt() + 1e-8)
                eigen_vec = (d_cur - d_cur_embedded) / (((d_cur - d_cur_embedded)*(d_cur - d_cur_embedded)).sum(dim=[1,2,3],keepdim=True).sqrt() + 1e-8)
            
                h = (t_cur - t_next)
                z = (w_stiff*h*stiffness)**2
                beta = (stiffness>w_con).float()                

                x_next = x_next - h * beta * z * (eigen_vec * d_cur).sum(dim=[1,2,3],keepdim=True) * eigen_vec
                            
            x_cur_embedded = x_next_Euler
            d_cur_embedded = d_prime
...
def dpm_2_sampler(
    ...
    **kwargs
    ...
    for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):                # 0, ..., N-1
        ...
        x_next_Euler = x_cur + (t_mid - t_cur) * d_cur

        # Apply 2nd order correction.
        denoised = get_denoised(net, x_next_Euler, t_mid, class_labels=class_labels, condition=condition, unconditional_condition=unconditional_condition)
        d_prime = (x_next_Euler - denoised) / t_mid
        x_next = x_cur + (t_next - t_cur) * ((1 / (2*r)) * d_prime + (1 - 1 / (2*r)) * d_cur)

        # Apply ERK-Guid
        if w_stiff > 0.0:
            stiffness = ((d_cur - d_prime)*(d_cur - d_prime)).sum(dim=[1,2,3],keepdim=True).sqrt() / (((x_cur - x_next_Euler)*(x_cur - x_next_Euler)).sum(dim=[1,2,3],keepdim=True).sqrt() + 1e-8)
            eigen_vec = (d_cur - d_prime) / (((d_cur - d_prime)*(d_cur - d_prime)).sum(dim=[1,2,3],keepdim=True).sqrt() + 1e-8)
...
def deis_sampler(
    ...
    **kwargs
    ...
    for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):                # 0, ..., N-1
        ...
        # Apply ERK-Guid
        if w_stiff > 0.0:
            if buffer_x_hat[-1] is not None:
                x_hat = x_cur
                x_hat_embedded = buffer_x_hat[-1]
                d_cur_embedded = buffer_d_cur[-1]
                
                stiffness = ((d_cur - d_cur_embedded)*(d_cur - d_cur_embedded)).sum(dim=[1,2,3],keepdim=True).sqrt() / (((x_hat - x_hat_embedded)*(x_hat - x_hat_embedded)).sum(dim=[1,2,3],keepdim=True).sqrt() + 1e-8)
                eigen_vec = (d_cur - d_cur_embedded)/(((d_cur - d_cur_embedded)*(d_cur - d_cur_embedded)).sum(dim=[1,2,3],keepdim=True).sqrt() + 1e-8)

                h = (buffer_t[-2] - buffer_t[-1])
                z = (w_stiff*h*stiffness)**2
                beta = (stiffness>w_con).float()

                x_next = x_next + h * beta * z * (eigen_vec * d_cur).sum(dim=[1,2,3],keepdim=True) * eigen_vec

        buffer_d_cur.append(d_cur)
        buffer_x_hat.append(x_cur)
