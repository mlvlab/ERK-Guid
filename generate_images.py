# Copyright (c) 2026, Inho Kong, Sojin Lee, Youngjoon Hong, Hyunwoo J. Kim. All rights reserved.
...
#----------------------------------------------------------------------------
# ERK-Guid with Heun sampler.
# "Error as Signal: Stiffness-Aware Diffusion Sampling via Embedded Runge-Kutta Guidance"

def edm_sampler_erk_guid(
    net, noise, labels=None, gnet=None,
    num_steps=32, sigma_min=0.002, sigma_max=80, rho=7, guidance=1,
    S_churn=0, S_min=0, S_max=float('inf'), S_noise=1,
    dtype=torch.float32, randn_like=torch.randn_like, w_stiff = 0.0, w_con = 0.0,
):
    if S_churn > 0:
        raise NotImplementedError("Stochastic sampling (S_churn > 0) is not currently supported with ERK-Guid. Please set S_churn=0.")
    ...
    def ERK_Guid(x_hat, t_hat, t_next, x_hat_embedded, d_cur_embedded):
        # Euler step.
        ...
        denoised  = denoise(x_next_Euler, t_next)
        d_prime = (x_next_Euler - denoised) / t_next
        x_next_Heun = x_hat + (t_next - t_hat) * (0.5 * d_prime + 0.5 * d_cur)

        # ERK-Guid requires ERK pair (Do not apply on first step)
        if x_hat_embedded is not None: 
            # stiffness estimator
            ...
            # Apply ERK-Guid
            h = (t_hat - t_next)
            z = w_stiff * h * stiffness
            beta = (stiffness > w_con).float()
            x_next_Heun = x_next_Heun - h * beta * (z*z) * (eigen_vec * d_cur).sum(dim=[1,2,3],keepdim=True) * eigen_vec

        return x_next_Heun, x_next_Euler, d_prime
    ...
    for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])): # 0, ..., N-1
        ...
        if i < num_steps - 1:
            # Heun update w/ ERK-Guid
            x_next, x_hat_embedded, d_cur_embedded = ERK_Guid(x_hat, t_hat, t_next, x_hat_embedded, d_cur_embedded)
        else:
            # Euler update.
...
def generate_images(
    ...
    sampler_fn          = edm_sampler_erk_guid,          # Which sampler function to use.
...
# hyperparameters for ERK-Guid
@click.option('--w_stiff', 'w_stiff',       help='Guidance scale for ERK-Guid', metavar='FLOAT',                         type=float, default=0.0, show_default=True)
@click.option('--w_con', 'w_con',       help='Guidance threshold for ERK-Guid', metavar='FLOAT',                         type=float, default=0.5, show_default=True)
